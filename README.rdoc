...
In this script when messages for /1/fader1 are received, the @i's instance variable @pitch is set:
  ...
  @i = Instrument.new(:join => true)
  
The :range option means that the received OSC value is translated from a number between 0 to 1 to the analogous value between 0 and 127
...
You can also do all of this using instance methods ala:
  ...
  i = Instrument.new
  i.osc_accessor :pitch, :pattern => "/1/fader1", :range => { :remote => 0..1, :local => 0..127 }
  i.osc_join
  
OR by using a Hash map such as:
  
  class Instrument; include OSCObject::Node; end
  ...
  i = Instrument.new(:send_ip => "192.168.1.5", :map => map, :join => true)
